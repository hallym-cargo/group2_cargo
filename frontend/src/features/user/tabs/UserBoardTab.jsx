...
export default function UserBoardTab({ controller }) {
  const { auth, filteredShipments, selectedId, setSelectedId, handleToggleBookmark, selected, roleTheme, offerForm, setOfferForm, handleCreateOffer, handleAcceptOffer, handleStart, handleCompletionProofChange, completionProof, handleComplete } = controller
    ...
    <div className="page-stack">
      ...
      <div className="detail-layout">
        ...
        <div className="surface side-form">
          {selected ? (<><SectionTitle title="역할별 액션" desc={`${roleText(auth.role)} 기준으로 표시됩니다.`} /><div className="surface-sub role-side-guide"><strong>{roleTheme?.label}</strong><p className="section-desc">{auth.role === 'SHIPPER' ? '화주는 입찰 비교와 차주 확정, 운행 확인이 핵심입니다.' : '차주는 입찰 등록, 운반 시작, ETA 기준 완료 전환이 핵심입니다.'}</p></div>{auth.role === 'DRIVER' && selected.status === 'BIDDING' && !selected.hasMyOffer && <div className="form-stack"><div className="surface-sub"><strong>정산 안내</strong><p className="section-desc">차주는 확정 금액에서 3% 수수료를 제외한 나머지 금액을 받습니다. 예를 들어 100,000원 제안이 확정되면 97,000원이 정산됩니다.</p></div><input placeholder="제안 금액" value={offerForm.price} onChange={(e) => setOfferForm({ ...offerForm, price: e.target.value })} /><textarea rows="4" placeholder="제안 메시지" value={offerForm.message} onChange={(e) => setOfferForm({ ...offerForm, message: e.target.value })} /><button className="btn btn-primary" onClick={handleCreateOffer}>입찰 제안</button></div>}{auth.role === 'DRIVER' && selected.status === 'BIDDING' && selected.hasMyOffer && <div className="surface-sub"><strong>이미 이 배차에 입찰했습니다.</strong><p className="section-desc">내 입찰 태그가 붙은 배차는 화주 선택 결과를 기다리면 됩니다.</p></div>}{auth.role === 'SHIPPER' && <div className="list-stack">{(selected.offers || []).length ? selected.offers.map(offer => <div key={offer.id} className="offer-card"><div className="detail-head"><strong>{offer.driverName}</strong><span className="badge badge-neutral">{offer.status}</span></div><div className="section-desc">평점 {formatRatingSummary(offer.driverAverageRating, offer.driverRatingCount)}</div>{offer.driverBio && <small>{offer.driverBio}</small>}<p>{formatCurrency(offer.price)}</p><small>{offer.message || '메시지 없음'}</small>{selected.status === 'BIDDING' && offer.status === 'PENDING' && <button className="btn btn-primary small" onClick={() => handleAcceptOffer(offer.id)}>이 차주 확정</button>}</div>) : <div className="empty-box small">등록된 제안이 없습니다.</div>}</div>}{auth.role === 'DRIVER' && selected.assignedDriverName === auth.name && <div className="form-stack">{selected.status === 'CONFIRMED' && <button className="btn btn-primary" onClick={handleStart}>운반 시작</button>}{selected.status === 'IN_TRANSIT' && <><div className="surface-sub"><strong>자동 이동 시뮬레이션</strong><p className="section-desc">운반 시작 시 예상 시간에 맞춰 트럭이 출발지에서 도착지까지 자동으로 이동합니다. 위치 입력 없이 지도에서 진행률과 남은 시간을 바로 확인할 수 있습니다.</p></div><div className="surface-sub"><strong>배송 완료 사진 등록</strong><p className="section-desc">완료 처리 전에 현장 사진 1장을 반드시 등록해야 합니다.</p><input type="file" accept="image/*" onChange={handleCompletionProofChange} />{completionProof.dataUrl && <div className="image-preview-row"><img src={completionProof.dataUrl} alt="completion-proof" className="image-preview-thumb" /></div>}</div><button className="btn btn-primary" onClick={handleComplete} disabled={!selected.tracking?.completable || !completionProof.dataUrl}>운송 완료</button>{!selected.tracking?.completable && <small>예상 도착 시간이 지나야 완료 가능합니다.</small>}{selected.tracking?.completable && !completionProof.dataUrl && <small>완료 사진을 등록하면 완료 버튼이 활성화됩니다.</small>}</>}</div>}</>) : <div className="empty-box">상세를 선택하면 액션이 표시됩니다.</div>}
        </div>
      </div>
    </div>
  )
