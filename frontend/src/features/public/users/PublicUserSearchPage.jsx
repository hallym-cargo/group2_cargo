      ...
      <section className="landing-info public-directory-hero">
        <div className="landing-info__inner">
          <div className="landing-sectionHead">
            ...
            <h2>{isDriver ? '가입된 차주를 한 번에 보고 이름으로 바로 찾을 수 있습니다.' : '가입된 화주를 한 번에 보고 이름으로 바로 찾을 수 있습니다.'}</h2>
            <p>헤더는 그대로 유지하고, 아래 탭에서 화주와 차주를 바로 전환할 수 있도록 구성했습니다.</p>
          </div>

          <div className="public-directory-tabs" role="tablist" aria-label="공개 사용자 역할 전환">
            <button
              className={role === 'SHIPPER' ? 'landing-filterChip active' : 'landing-filterChip'}
            ...
            <button
              className={role === 'DRIVER' ? 'landing-filterChip active' : 'landing-filterChip'}
          ...
          <div className="public-directory-search surface">
            <div className="public-directory-search__meta">
              ...
              <small>{isLoading ? '목록을 갱신하는 중입니다.' : `총 ${users.length}명`}</small>
            </div>
            <div className="public-directory-search__controls">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') controller.searchPublicUsers(role, controller.publicUserKeyword) }}
              />
              <button className="landing-btn landing-btn--primary" onClick={() => controller.searchPublicUsers(role, controller.publicUserKeyword)}>검색</button>
              <button className="landing-btn landing-btn--light" onClick={() => controller.resetPublicUserSearch(role)}>전체 보기</button>
            </div>
          </div>

          <div className={`public-directory-results ${isLoading ? 'is-loading' : ''}`}>
            {isLoading && <PublicSectionLoading text={`${isDriver ? '차주' : '화주'} 데이터를 불러오는 중입니다...`} />}
