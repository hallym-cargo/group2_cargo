...
export default function AdminRatingsTab({ controller }) {
  ...
  return (
    <div className="page-stack">
      ...
      <div className="surface"><SectionTitle title="최근 등록 평점" desc="화주와 차주가 서로에게 남긴 최신 평가입니다." /><table className="board-table compact"><thead><tr><th>화물</th><th>평가자</th><th>대상</th><th>점수</th><th>코멘트</th><th>일시</th></tr></thead><tbody>{adminRecentRatings.map(item => <tr key={item.id}><td>{item.shipmentTitle}<small>#{item.shipmentId}</small></td><td>{item.fromUserName}</td><td>{item.toUserName}</td><td>{renderStars(item.score)} ({item.score})</td><td>{item.comment || '-'}</td><td>{formatDate(item.createdAt)}</td></tr>)}</tbody></table></div>
    </div>
  )
}
