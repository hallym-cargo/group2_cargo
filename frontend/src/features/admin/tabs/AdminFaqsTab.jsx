...
export default function AdminFaqsTab({ controller }) {
  const { faqForm, setFaqForm, submitFaq, editingFaqId, setEditingFaqId, adminFaqs, deleteAdminFaq, loadAdmin, loadPublic } = controller
  return (
    <div className="page-stack">
      <div className="surface form-surface"><SectionTitle title="FAQ 작성 / 수정" /><div className="form-stack"><input placeholder="카테고리" value={faqForm.category} onChange={(e) => setFaqForm({ ...faqForm, category: e.target.value })} /><input placeholder="질문" value={faqForm.question} onChange={(e) => setFaqForm({ ...faqForm, question: e.target.value })} /><textarea rows="5" placeholder="답변" value={faqForm.answer} onChange={(e) => setFaqForm({ ...faqForm, answer: e.target.value })} /><input placeholder="정렬 순서" value={faqForm.sortOrder} onChange={(e) => setFaqForm({ ...faqForm, sortOrder: e.target.value })} /><div className="table-actions"><button className="btn btn-primary" onClick={submitFaq}>저장</button>{editingFaqId && <button className="btn btn-ghost" onClick={() => { setEditingFaqId(null); setFaqForm(emptyFaq) }}>취소</button>}</div></div></div>
