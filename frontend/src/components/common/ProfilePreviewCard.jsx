...
export default function ProfilePreviewCard({ title, profile, onProfileClick }) {
  if (!profile) return null

  const clickable = !!onProfileClick && !!profile.id
  ...
  return (
    <div className="surface-sub">
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        <button
          type="button"
          className={clickable ? 'profile-preview-trigger' : 'profile-preview-trigger is-static'}
          onClick={() => clickable && onProfileClick(profile.id, profile)}
        >
