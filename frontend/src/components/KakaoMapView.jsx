...
const KAKAO_APP_KEY = import.meta.env.VITE_KAKAO_MAP_APP_KEY || '99887cb3e304bd757a742ebcd0f55eb7'
...
function getProgress(shipment, now = Date.now()) {
  ...
  if (shipment.status !== 'IN_TRANSIT' || !shipment.startedAt || !shipment.estimatedArrivalAt) return 0

  const started = new Date(shipment.startedAt).getTime()
  const eta = new Date(shipment.estimatedArrivalAt).getTime()
  if (!Number.isFinite(started) || !Number.isFinite(eta) || eta <= started) return 0
...
function getTrackingMeta(shipment, now = Date.now()) {
  ...
  const remainingMinutes = shipment.status === 'COMPLETED'
    ? 0
    : shipment.status === 'IN_TRANSIT' && shipment.estimatedArrivalAt
      ? Math.max(0, Math.ceil((new Date(shipment.estimatedArrivalAt).getTime() - now) / 60000))
      : shipment.tracking?.remainingMinutes ?? shipment.estimatedMinutes

  const roughLocation = shipment.status === 'COMPLETED'
    ? '도착 완료'
...
function buildCumulativeDistances(routeCoords) {
  ...
  for (let i = 1; i < routeCoords.length; i += 1) {
    cumulative.push(cumulative[i - 1] + getDistanceMeter(routeCoords[i - 1], routeCoords[i]))
  }
  return cumulative
}
...
export default function KakaoMapView({ shipment }) {
  ...
  const [routeState, setRouteState] = useState({ loading: false, coords: [], cumulative: [], totalDistanceMeter: 0, totalTimeSecond: 0 })
  ...
  const trackingMeta = useMemo(() => getTrackingMeta(shipment, nowTick), [shipment, nowTick])
  ...
  useEffect(() => {
    ...
    async function loadRoute() {
      ...
      try {
        ...
        console.error(e)
        if (!cancelled) {
          setError(e?.response?.data?.message || e.message || '도로 경로를 불러오지 못했습니다.')
          setRouteState({ loading: false, coords: [], cumulative: [], totalDistanceMeter: 0, totalTimeSecond: 0 })
        }
      }
    }
  ...
  }, [shipment?.id, shipment?.originLat, shipment?.originLng, shipment?.destinationLat, shipment?.destinationLng])
  ...
  const points = useMemo(() => {
    ...
    const fallbackCurrent = shipment.status === 'COMPLETED' ? destination : origin
    const current = routeState.coords.length
      ? getPositionByProgress(routeState.coords, routeState.cumulative, trackingMeta.progress)
      : fallbackCurrent
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const current = routeState.coords.length
        ? getPositionByProgress(routeState.coords, routeState.cumulative, progress)
        : points.current
  ...
  return (
    <div className="map-panel">
      ...
      <div className="map-meta-grid">
        <div><span>진행률</span><strong>{Math.round((trackingMeta?.progress || 0) * 100)}%</strong></div>
        <div><span>남은 시간</span><strong>{trackingMeta?.remainingMinutes ?? shipment?.estimatedMinutes}분</strong></div>
        <div><span>현재 상태</span><strong>{trackingMeta?.roughLocation || '출발 전'}</strong></div>
        <div><span>사용자 지도 조작</span><strong>{Date.now() < pauseUntilRef.current ? '7초 일시정지' : '실시간 추적중'}</strong></div>
      </div>
    </div>
  )
}
