...
public class AuthService {
	...
	public AuthDtos.AuthResponse signUp(AuthDtos.SignUpRequest request) {
		...
		User user = User.builder().email(request.getEmail()).password(passwordEncoder.encode(request.getPassword()))
				.name(request.getName()).phone(request.getPhone()).companyName(request.getCompanyName())
				.vehicleType(request.getVehicleType()).role(role).status(UserStatus.ACTIVE).build();
	...
	public AuthDtos.AuthResponse kakaoLogin(AuthDtos.KakaoLoginRequest request) {
		...
		KakaoUserInfo kakaoUser = resolveKakaoUserInfo(request);

		String kakaoEmail = "kakao_" + kakaoUser.kakaoId() + "@kakao.local";

		String name = kakaoUser.nickname() != null && !kakaoUser.nickname().isBlank() ? kakaoUser.nickname() : "카카오사용자";
		...
		User user = User.builder().email(kakaoEmail).password(passwordEncoder.encode("KAKAO_USER")).name(name)
				.role(request.getRole()).status(UserStatus.ACTIVE).profileImageUrl(kakaoUser.profileImageUrl())
				.profileCompleted(false).build();
	...
	private User createKakaoUser(KakaoUserInfo kakaoUserInfo, UserRole requestedRole) {
		...
		User user = User.builder().kakaoId(kakaoUserInfo.kakaoId()).email(kakaoLocalEmail)
				.password(passwordEncoder.encode("KAKAO_LOGIN_USER_" + kakaoUserInfo.kakaoId())).name(nickname)
				.role(role).status(UserStatus.ACTIVE).profileImageUrl(kakaoUserInfo.profileImageUrl())
				.contactEmail(null).profileCompleted(false).build();
	...
	private KakaoUserInfo resolveKakaoUserInfo(AuthDtos.KakaoLoginRequest request) {
		...
		return new KakaoUserInfo(request.getKakaoId(), request.getNickname(), request.getProfileImage());
	}
	...
	private KakaoUserInfo getKakaoUserInfo(String accessToken) {
		try {
			...
			ResponseEntity<String> response = restTemplate.exchange(KAKAO_USER_INFO_URL, HttpMethod.GET,
